...
export class BrowserFlowService {
  ...
  async listServices(): Promise<Array<{ name: string; available: boolean; metadata?: unknown }>> {
    let enrolledNames = this.config.ENS_PROVIDER_NAMES;
    try {
      ...
      if (enrolled.rows.length > 0) {
        enrolledNames = enrolled.rows.map((row) => row.ens_name);
      }
    } catch {
      // DB offline fallback
    }
    return Promise.all(enrolledNames.map(async (ens_name) => {
      try {
  ...
  async preview(maxSpendTinybarsValue: string, capability: Capability): Promise<RoutePreview> {
    ...
    return createRoutePreview(this.config, enrolledNames, maxSpendTinybars, capability);
  }
