...
- A second, independently generated synthetic fixture completed the direct live workflow on its first paid attempt. ENS selected `alpha.ocr.agentpayapp.eth`; the x402 requirements matched its live metadata and offer; Hedera Testnet transaction `0.0.7162784@1789123959.119920599` settled `1000000` tinybars from payer `0.0.10463298` to Alpha recipient `0.0.10463387`; and Gemini 3.6 returned all six ground-truth fields exactly: invoice `SP-2026-2042`, date `2026-09-11`, currency `EUR`, subtotal `241.50`, tax `48.30`, and total `289.80`. Supabase records request `d09b2d03-8d35-4078-87c0-b316c361adbe` as `SUCCEEDED` with payment `SETTLED` and reservation `CONSUMED`.
- The pre-payment readiness phase now retries read-only provider offer requests for up to 90 seconds, refreshes ENS after wake-up, fetches a fresh offer, and validates exact unsigned 402 terms before any run creation, reservation, or signing. A live dry-run exercised the retry path: Alpha became ready on attempt 2 after approximately 32 seconds, after which fresh ENS/offer/402 validation passed with no payment or inference.
...
`fixtures/synthetic-invoice-2.svg` is a deterministic repository-authored test document rendered locally to `fixtures/synthetic-invoice-2.png` with the already installed project-local Sharp dependency. Its expected fields are stored in `fixtures/synthetic-invoice-2.expected.json`. It is visibly marked synthetic, contains only fictional entities and values, and has no account details, QR codes, or payment instructions.
