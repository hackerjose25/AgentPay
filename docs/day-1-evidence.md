...
- The local extraction adapter and Render configuration now target Gemini 3.6 Flash and validate image signatures, dimensions, size limits, and structured model output. Authenticated model metadata returns HTTP 200, but the correction is not yet deployed and has not made an inference. The single authorized diagnostic request proved Gemini 2.5 Flash returns HTTP 404 for this new API project.
- The first paid request produced no extraction result because the deployed provider returned HTTP 500 after settlement. Its database request is `FAILED` with redacted code `PAID_EXTRACTION_FAILED`, while the payment remains `SETTLED` and its reservation `CONSUMED`. Do not issue a second payment; diagnose and recover the original request.
